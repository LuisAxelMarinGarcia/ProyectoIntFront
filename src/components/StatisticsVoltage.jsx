  ...
  useEffect(() => {
    if (isLoggedIn) {
      const fetchData = async () => {
        try {
          const mediaResponse = await axios.get('http://3.224.141.144/api/voltsensor/media');
          const desviacionResponse = await axios.get('http://3.224.141.144/api/voltsensor/standardDeviation');
          const varianzaResponse = await axios.get('http://3.224.141.144/api/voltsensor/variance');
...
export default StatisticsVoltage;
